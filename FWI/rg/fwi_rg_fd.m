function [seis, laps] = fwi_rg_fd(nz, nx, nt, dz, dx, dt, npml, v, wlt, sxz, gxz, varargin)
...
sp0 = zeros(nze,nxe);
sp1 = zeros(nze,nxe);
laps = zeros(nze, nxe, nt);

seis = zeros(nt, ng);
for it = 1:nt
  
    [sp0, lap] = step_fd(sp0, sp1, v2, nze, nxe, ne, dz, dx, dt, C, 0);
    sp0 = add_source(sp0, wlt(it), s, 0);
    %%% Instead of outputing wavefield, we output the laplacian for
    %%% computing gradient
